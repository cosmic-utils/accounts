...
- **`src/proxy.rs`** — zbus `#[proxy]` trait definitions consumed by clients: `Accounts` (interface `dev.edfloreshz.Accounts.Account`, well-known bus name `dev.edfloreshz.Accounts`) for account management and capability advertisement (`list_providers`); `Calendar`, `Todo`, and `Mail` for the per-account service objects (all `default_service = "dev.edfloreshz.Accounts"`, the daemon's actual bus name); and `Provider1` (interface `dev.edfloreshz.Accounts.Provider1`) implemented by each provider process — `get_user_info` and `get_service_config`, both provider-agnostic on the daemon side.
- **`src/clients/`** — typed async wrappers over the proxies, re-exported as `AccountsClient`:
  ...
  - `calendar.rs` / `todo.rs` / `mail.rs` — `CalendarClient`, `TodoClient`, `MailClient`: per-account proxies at `/dev/edfloreshz/Accounts/{Calendar,Todo,Mail}/{account.dbus_id()}`.
...
- **`main.rs`** — entry point. Starts an axum HTTP server on `127.0.0.1:49173` (`CALLBACK_PORT`, picked from the IANA dynamic/private range to avoid colliding with common dev ports like 8080) with a `/callback` route for the OAuth2 redirect, loads the `ProviderRegistry` into the static `REGISTRY`, opens a zbus session connection, registers the `AccountsInterface` D-Bus object under the well-known name `dev.edfloreshz.Accounts`, then instantiates and registers per-service D-Bus objects for each persisted, enabled account via `ServiceFactory`. `handle_callback` exchanges the OAuth `code`/`state`, completes auth via `AccountsClient`, emits `account_added`/`account_exists` signals, and returns an HTML result page.
...
- **`services/`** — per-service D-Bus objects implementing the core `AccountService` trait, with GOA-style property names (e.g. `ImapHost`, `SmtpAuthXoauth2`):
  - `mod.rs` — `ServiceFactory` creates and registers service objects for an account: `Calendar`, `Mail` (`Service::Email`), and `Todo` are wired up. `contacts.rs` still exists but is commented out and still references the old provider enum — not touched by this pass, only `Calendar`/`Mail`/`Todo` were migrated to the provider-plugin model.
  - `calendar.rs` — `CalendarService`, interface `dev.edfloreshz.Accounts.Calendar`. `uri`/`accept_ssl_errors` come from a live `Provider1::get_service_config("calendar")` D-Bus call to the account's provider process, resolved via `REGISTRY`.
  - `todo.rs` — `TodoService`, interface `dev.edfloreshz.Accounts.Todo`, same pattern via `get_service_config("todo")` (Google Tasks API / Microsoft To Do endpoint).
  - `mail.rs` — `MailService`, interface `dev.edfloreshz.Accounts.Mail`. IMAP/SMTP host and protocol properties come from `get_service_config("email")`; identity properties (`EmailAddress`, `Name`, `ImapUserName`, `SmtpUserName`) are answered locally from the account's own `email`/`display_name` — no provider round trip needed for data the daemon already has.
...
- `contacts.rs` is still commented out in `accounts-daemon/src/services/mod.rs` and still references the old provider enum, so it won't compile as-is if re-enabled. Calendar, Mail, and Todo were migrated to the provider-plugin model; Contacts was not (not requested, and CardDAV contact sync is a larger scope than the other three).
...
- Mail/Todo property getters each make their own `Provider1::get_service_config` D-Bus round trip (mirroring the pattern already established by `CalendarService`), so reading all of `MailService`'s ~14 properties means ~14 round trips to the provider process. Fine for now given provider processes are local and cheap to call, but a candidate for a per-request config cache if it becomes a bottleneck.

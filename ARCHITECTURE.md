...
- `accounts-ui` currently fails to build against the pinned `libcosmic` git dependency for reasons unrelated to this work (`Subscription::run_with_id` renamed, `widget::column()` now requires an argument, `widget::horizontal_space`/`widget::row::row` moved) — pre-existing API drift from tracking `libcosmic`'s main branch, not something introduced by the provider-plugin migration.
